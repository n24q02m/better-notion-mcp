    ...
    inputSchema: {
      ...
      properties: {
        ...
        icon: {
          type: 'string',
          description:
            'Icon: emoji (e.g. "clip"), external URL (https://...), or built-in shorthand (name:color, e.g. "document:gray")'
        },
        cover: {
          type: 'string',
          description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        icon: {
          type: 'string',
          description:
            'Icon (for update_database): emoji (e.g. "clip"), external URL (https://...), or built-in shorthand (name:color, e.g. "document:gray")'
        },
        cover: {
          type: 'string',
          description:
...
// Pre-compute tool names for O(1) validation during tool calls
const TOOL_NAMES = TOOLS.map((t) => t.name)
const HELP_TOOL_NAMES = TOOL_NAMES.filter((name) => name !== 'help')

...
export function extractPageProperties(pageProperties: any): any {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // ⚡ Bolt: Cache property lookup inside tight loop.
    // Looking up `p.type` up to 21 times per property across heterogeneous objects
    // is expensive. Caching it reduces property access overhead by ~80%.
    const type = p.type

...
export function extractPageProperties(pageProperties: any): any {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // Cache the property type locally to prevent redundant object property lookups in
    // the hot evaluation path of the massive if-else branch chain below.
    const type = p.type
    ...
    if (type === 'title' && p.title) {
      ...
      for (let j = 0; j < files.length; j++) arr[j] = files[j].file?.url || files[j].external?.url || files[j].name
      properties[key] = arr

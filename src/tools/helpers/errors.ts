...
function sanitizeErrorDetails(error: unknown): Record<string, unknown> {
  ...
  if (error && typeof error === 'object' && 'status' in error) safe.status = (error as any).status
  if (error && typeof error === 'object' && 'response' in error && (error as any).response?.status)
    safe.status = (error as any).response.status
...
function stripSensitiveFields(obj: unknown, seen = new WeakSet()): void {
  ...
  if (o.request) {
    redactHeaderMap(o.request.headers)
    redactHeaderMap(o.request._headers)
  }
  if (o.config) {
    redactHeaderMap(o.config.headers)
  }
  if (o.response) {
    redactHeaderMap(o.response.headers)
  }
...
function mapNetworkError(error: unknown): NotionMCPError | null {
  if (
    ...
    ((error as any).message.includes('ECONNREFUSED') || (error as any).message.includes('ENOTFOUND'))
  ) {
...
function mapValidationError(error: unknown): NotionMCPError | null {
  if (!error || typeof error !== 'object' || (error as any).code !== 'validation_error') return null

  const bodyMessage: string = (error as any).body?.message || ''
  ...
  return new NotionMCPError(
    ...
    'VALIDATION_ERROR',
    suggestion,
    sanitizeValidationBody((error as any).body)
  )
}
...
function mapNotionError(error: unknown): NotionMCPError | null {
  ...
  const message = (error as any).message || 'Unknown Notion API error'
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        ...
        typeof error === 'object' &&
        'code' in error &&
        ((error as any).code === 'UNAUTHORIZED' || (error as any).code === 'NOT_FOUND')
      ) {
        throw enhanceError(error)
      }

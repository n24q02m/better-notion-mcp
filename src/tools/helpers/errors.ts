 ...
 */
/**
 * Keywords that indicate a property format error in Notion API validation errors
 */
...
function handleNotionError(error: any): NotionMCPError {
  ...
  if (code === 'validation_error') {
    ...
    let hasPropertyFormatError = false
    for (const keyword of PROPERTY_FORMAT_KEYWORDS) {
      if (bodyMessage.includes(keyword)) {
        hasPropertyFormatError = true
        break
      }
    }

    if (hasPropertyFormatError) {
      suggestion =
 ...
 */
/**
 * Map of error codes to suggestion strings
 */

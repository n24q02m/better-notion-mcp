...
export async function autoPaginate<T>(
  ...
  do {
    ...
    // BOLT OPTIMIZATION: Constrain loop boundary directly to prevent intermediate allocations via .slice()
    const len = limit > 0 ? Math.min(results.length, limit - allResults.length) : results.length
    for (let i = 0; i < len; i++) {

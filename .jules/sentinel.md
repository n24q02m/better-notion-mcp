...
## 2026-09-02 - Prevent Path Traversal Regex Overblocking
**Vulnerability:** A path traversal check in the KV outbound interceptor used `key.includes('/..')`, which incorrectly blocked legitimate keys containing `/..` followed by other characters (e.g. `better-notion/..foo`).
**Learning:** Broad string inclusion checks for directory traversal markers (`/..`) are prone to false positives when they don't explicitly require path boundaries.
**Prevention:** When checking for path traversal, always anchor checks to path boundaries (e.g. `key.includes('/../') || key.endsWith('/..') || key === '..'`) rather than generic substring matches to avoid breaking valid application inputs.

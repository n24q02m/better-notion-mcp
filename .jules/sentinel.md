...
## 2026-07-28 - Improve XPIA JSON-Escaped Padding Sanitization
**Vulnerability:** The `wrapToolResult` regex `/<(?:[\s/])*untrusted_notion_content/gi` failed to sanitize untrusted content when attackers included JSON-escaped whitespaces or newlines (e.g. `\n`) between the opening bracket and the tag name in a stringified JSON payload.
**Learning:** Leniency in XML/HTML parsing (including LLMs parsing tags) allows optional whitespaces/newlines. When matching tags inside a stringified JSON payload, standard regex whitespace markers (`\s`) fail because the newline is actually the two literal characters `\` and `n`.
**Prevention:** Always explicitly account for JSON-escaped sequences alongside optional spaces/slashes using two backslashes in regex literals (e.g., `/<(?:[\s/]|\\[nrtfb]|\\u[0-9a-fA-F]{4})*tag/gi`) to safely handle padding and evasion tactics in unparsed JSON context.

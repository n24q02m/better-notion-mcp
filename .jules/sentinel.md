...
## 2026-09-04 - False Positive in KV Path Traversal Check
**Vulnerability:** The outbound KV handler (`kvOutbound` in `worker.ts`) used a generic `.includes('/..')` string check to prevent directory traversal. This check was overly broad, as it would falsely block legitimate paths that simply contain `..` without actually being a directory traversal (e.g., `better-notion/..name`).
**Learning:** String matching for path traversal detection must be precise and account for URL boundaries. Catch-all substring checks like `.includes('/..')` frequently cause false positives for valid names.
**Prevention:** Use exact boundary conditions (e.g., `.includes('/../') || .endsWith('/..') || === '..'`) to accurately block actual traversal sequences without restricting valid path components.

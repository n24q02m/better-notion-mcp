...
## 2025-01-01 - Overly Broad Path Traversal Check False Positive
**Vulnerability:** The application was rejecting legitimate KV keys starting with `..` (e.g. `better-notion/..name`) because it used a broad `includes('/..')` string check.
**Learning:** Checking for path traversal strictly using `includes('/..')` leads to false positives, blocking valid user input without actually adding more security beyond what a strict boundary check would provide.
**Prevention:** Use exact boundary conditions to validate path traversal: `includes('/../') || endsWith('/..') || key === '..'` to allow valid prefixes like `..name` while preventing actual directory escape.

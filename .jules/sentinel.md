...
## 2023-10-27 - False Positive in Path Traversal Prevention
**Vulnerability:** The `kvOutbound` handler in `src/worker.ts` used `key.includes('/..')` to block path traversal in KV keys. However, this is overly broad and incorrectly blocks valid keys where a segment begins with `..` (e.g., `better-notion/..name`).
**Learning:** Using generic substring matches (like `includes('/..')`) for path traversal prevention is prone to false positives because it doesn't respect segment boundaries.
**Prevention:** Use precise boundary checks (e.g., `key.includes('/../') || key.endsWith('/..') || key === '..'`) that strictly target traversal sequences without catching legitimate, similar-looking names.

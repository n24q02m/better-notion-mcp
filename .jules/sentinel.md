...
## 2024-05-24 - Case-insensitive Authorization header redaction
**Vulnerability:** The error sanitization utility `stripSensitiveFields` correctly deleted hardcoded `Authorization` and `authorization` properties, but failed to strip other case variations (e.g. `AUTHORIZATION`, `AuThOrIzAtIoN`) due to exact-match logic. This could lead to token leakage in logs if downstream modules generated headers with mixed casing.
**Learning:** Upstream libraries (e.g., node HTTP vs Axios) and proxy servers often mutate the casing of HTTP headers. Security redaction rules targeting specific keys must always evaluate the key in a case-insensitive manner to prevent bypasses.
**Prevention:** Use a helper loop with `key.toLowerCase()` to match and delete target keys rather than relying on exact dot-notation or bracket-notation property deletions.

...
## 2026-06-21 - Path Traversal via basename('..') using '-hacked' technique
**Vulnerability:** Even though the path traversal via `basename('..')` was previously addressed using `fullPath.startsWith(DOCS_DIR)`, a secondary vulnerability existed because `startsWith` without an explicit directory separator allows attackers to access adjacent directories sharing a prefix. For instance, accessing a file in `src/docs-hacked/` would be permitted because `'/app/src/docs-hacked/file.txt'.startsWith('/app/src/docs')` evaluates to `true`.
**Learning:** Checking directory bounds using `startsWith` with only a string path prefix is vulnerable to adjacent directory attacks when an explicit path separator is omitted.
**Prevention:** Always append a platform-specific path separator (`sep`) to the base directory path string before performing a `startsWith` check to ensure strict directory containment.

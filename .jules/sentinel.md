...
## 2025-02-14 - Fix JSON-escaped Newline XPIA Evasion in wrapToolResult
**Vulnerability:** XPIA (Cross-Prompt Injection Attack) evasion in Notion API output wrapping.
**Learning:** The `wrapToolResult` function sanitizes malicious XPIA closing tags using a regex (`/<[\s/]*untrusted_notion_content/gi`). However, because the input string `jsonText` is stringified JSON, newlines injected by an attacker (e.g. `<\n/untrusted_notion_content>`) are encoded literally as `\n` (backslash + `n`). The regex `[\s/]*` matches actual whitespace but not literal escaped sequences, allowing the attacker's closing tag to bypass sanitization. When the LLM parses the string back into JSON, the newline is interpreted correctly, resulting in a successful breakout from the `<untrusted_notion_content>` envelope.
**Prevention:** Always account for JSON-escaped control sequences when running regex replacements against stringified JSON payloads. Ensure the regex explicitly matches sequences like `\n`, `\r`, `\t`, `\f`, `\b`, and Unicode escapes (`\uXXXX`) in addition to raw whitespace.

...
## 2026-08-01 - Fix XPIA Breakout via Escaped Whitespace Padding
**Vulnerability:** The previous `wrapToolResult` regex `/<[\s/]*untrusted_notion_content/gi` failed to match JSON-escaped sequences (like `\n` or `\u0020`). If an attacker returned untrusted Notion content as a JSON string containing these sequences between the angle bracket and the tag name, the sanitization regex bypassed the breakout attempt.
**Learning:** Leniency in XML/HTML parsing (such as LLMs processing prompt pseudo-XML tags) extends past raw characters; since data often flows through JSON, prompt-injection defenses must explicitly sanitize literal string representations of whitespace (escaped backslash characters like `\n`).
**Prevention:** When sanitizing strings directly received from unparsed JSON against tag injection, always utilize a regex pattern that explicitly neutralizes JSON-escaped variations of valid padding alongside traditional whitespace matching.

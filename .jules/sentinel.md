...
## 2025-05-28 - XPIA Breakout via JSON-Escaped Padding
**Vulnerability:** Untrusted content returned from external APIs was wrapped in `<untrusted_notion_content>` tags. The regex used to sanitize payload breakout attempts (`/<[\s/]*untrusted_notion_content/gi`) checked for exact whitespace padding. However, when parsing untrusted JSON-stringified payloads, the attacker could exploit JSON-escaped character bypasses (e.g., `<\n/untrusted_notion_content>`). In stringified JSON, this sequence acts as two literal characters (backslash and 'n') rather than a true newline, bypassing the `\s` filter but acting as a newline when re-evaluated.
**Learning:** Security boundaries relying on XML/HTML-style tags within JSON payloads must sanitize JSON-escaped variations of expected strings because parsers evaluate these as single tokens, while raw regex sanitization acts on the literal unescaped strings.
**Prevention:** Extend strict spacing bounds to natively check for typical JSON escapes using non-capturing groups (e.g., `(?:[\s/]|\\[nrtfb]|\\u[0-9a-fA-F]{4})*`) rather than simple whitespace bounds (`\s*`).

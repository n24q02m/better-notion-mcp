...
## YYYY-MM-DD - Fix overly broad path traversal filter
**Vulnerability:** The existing path traversal filter in KV mapping (`includes('/..')`) incorrectly blocked valid directory or file names starting with `..` (e.g., `better-notion/..name`).
**Learning:** Using `includes('/..')` is overly broad. Path traversal checks must strictly validate standard boundaries to prevent false positives and block valid application data access.
**Prevention:** Always use strict path traversal boundary checks like `includes('/../') || endsWith('/..') || key === '..'` rather than generic substring matches that block valid strings starting with two dots.

...
## 2026-08-11 - XPIA Breakout via JSON-escaped whitespace
**Vulnerability:** Untrusted content tools were protected against XPIA using a closing tag replacement regex that did not match JSON-escaped whitespace (e.g., `\n`, `\r`, `\t`). Since the `wrapToolResult` function processes stringified JSON, an attacker could supply input with real newlines that `JSON.stringify` converts into `\n` (a literal backslash and `n`), successfully bypassing the sanitization regex but still being evaluated as a valid XML tag boundary by the LLM after parsing.
**Learning:** Security boundaries relying on regex matching over stringified JSON must explicitly account for JSON-escaped sequences representing whitespace, as standard regex whitespace classes (like `\s`) do not match these escaped sequences.
**Prevention:** When matching or sanitizing tags in stringified JSON, augment the whitespace regex to include JSON-escaped whitespace characters (e.g., `/\[nrtfb]/` for literals or `/\\[nrtfb]/` for `RegExp` constructors).

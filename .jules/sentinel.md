...
## 2025-05-29 - XPIA Breakout via JSON-escaped Padding
**Vulnerability:** Untrusted content tools were protected against XPIA using a tag replacement regex (`/<[\s/]*untrusted_notion_content/gi`) that only matched standard whitespaces and slashes. However, because the payload is processed as stringified JSON, newlines and tabs are encoded as literal backslash sequences (e.g., `\n`, `\t`). An attacker could pad their malicious breakout tag with JSON-escaped whitespace characters (e.g., `<\\n/untrusted_notion_content>`), successfully bypassing the sanitization layer and executing prompt injection.
**Learning:** When sanitizing stringified JSON payloads (or other serialized formats) using regex, standard character classes like `\s` do not match the literal escaped sequences (`\n`, `\t`, etc.). Attackers can leverage the serialization layer to obfuscate their padding.
**Prevention:** Always account for serialization escaping when validating or sanitizing serialized data using regex. For JSON-escaped sequences, explicitly match literal backslashes combined with their respective control characters or unicode escapes (e.g., `/<(?:[\s/]|\\[nrtfb]|\\u[0-9a-fA-F]{4})*.../gi`).

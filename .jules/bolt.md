...
## 2026-08-09 - Avoid .slice() for Limit Constraints on Accumulated Arrays\n**Learning:** When accumulating a large paginated set into an array and applying a strict limit (e.g., in `autoPaginate`), using `allResults.slice(0, limit)` creates an unnecessary intermediate array and wastes CPU cycles iterating over elements that will be discarded. In hot paths, this can cause measurable GC pressure.\n**Action:** Instead of accumulating everything and slicing, calculate the exact number of remaining items needed (`limit - allResults.length`) and directly constrain the loop boundary of the `.push()` operation (e.g., via `Math.min(results.length, limit - allResults.length)`).

...
## 2023-10-27 - Caching local property lookup speeds up Notion property extraction\n**Learning:** Notion page responses have deeply nested properties with varying schemas. Iterating over keys and repeatedly checking `p.type` multiple times per object inside an extensive if/else chain creates a significant performance bottleneck due to redundant object access latency. \n**Action:** Always extract and cache the property lookups (like `const type = p.type`) into local variables before performing large conditional evaluations in tight loops.

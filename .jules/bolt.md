...
## 2026-07-24 - Rejecting Micro-Optimization Churn
**Learning:** Reversing previous performance improvements (e.g., oscillating between pre-allocated arrays and string concatenation in `extractPageProperties`) without verifiable benchmarks causes churn and pollutes the review process. Theoretical micro-optimizations that contradict established learnings are likely to be rejected.
**Action:** Do not submit micro-optimizations on code paths that have already been deliberately structured (e.g., using pre-allocated arrays) unless you can provide quantitative benchmark evidence proving a measurable gain.

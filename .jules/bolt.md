...
## 2024-07-25 - Push limit constraint down into network I/O paginators
**Learning:** Fetching 100 pages of a paginated API only to truncate them in-memory to 10 items via `.slice()` is a massive, unforced performance bottleneck. When utilizing paginators (like `autoPaginate`), always forward the `limit` configuration down into the paginator loop so it can dynamically break early and avoid N+1 requests, instead of slicing arrays later.
**Action:** Always check paginated loops to ensure requested limits dynamically cap the fetch cycle. Additionally, use in-place array truncation (`arr.length = limit`) for local arrays instead of `.slice()` to avoid intermediate allocations and reduce GC pressure.

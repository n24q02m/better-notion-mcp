...
## 2025-03-03 - Optimize autoPaginate limit handling
**Learning:** In the paginated Notion API wrapper (`autoPaginate`), slicing an array at the end (`allResults.slice(0, limit)`) requires fetching all underlying pages first. This is an O(N) network bottleneck where N is the total pages, meaning thousands of items might be fetched when only a small `limit` is requested. Pushing the limit logic into `autoPaginate` and explicitly setting the `page_size` dynamically inside the fetching loop converts this to O(1) network overhead. Additionally, using in-place truncation (`allResults.length = limit`) avoids intermediate array allocations from `.slice()`, reducing GC overhead.
**Action:** When working with pagination or fetching wrappers, always push limit constraints as far down into the network call parameters (`page_size`) as possible. Prefer in-place array mutation over returning slices to optimize memory in hot paths.

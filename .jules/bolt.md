...
## 2024-07-25 - Stop Early Pagination by Leveraging Limit
**Learning:** In Notion API tools like `databases.ts` and `file-uploads.ts`, fetching data inside `autoPaginate` without passing the user's `limit` forces the application to fetch *all* pages of data from the API into memory, only to slice off the majority of the array afterward.
**Action:** Always pass the user-provided `limit` into `autoPaginate` as an option (e.g., `{ limit: input.limit }`) and ensure the underlying callback accepts and utilizes the dynamically optimized `pageSize` argument to halt unnecessary network requests. Additionally, utilize in-place array truncation (`arr.length = limit`) instead of `.slice()` to avoid intermediate allocations.
